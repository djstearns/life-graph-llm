...
with st.sidebar:
    ...
    st.sidebar.header("Generate Json")    
     ...
     # Add Wikipedia form in the sidebar
    st.sidebar.subheader("Fetch Wikipedia Content")
    wikipedia_url = st.sidebar.text_input("Wikipedia URL")
    fetch_wikipedia_button = st.sidebar.button("Fetch Wikipedia Content")

    if fetch_wikipedia_button and wikipedia_url:
...
st.write("This Page has two sections: The first is your current draft of Current Json Data, the second is a form that generates a json string that you can use to create your life graph. The third section is a form that fetches content from a wikipedia page. You can use the content to generate a json string for your life graph. ")
...
if 'wikipedia_content' in st.session_state:
    wikipedia_content = st.session_state["wikipedia_content"]
    st.text_area("Wikipedia Content", value=wikipedia_content, height=400)
